...
| P1-014D / P1-014F / R7-002 | SHACL: hierarchy inverse/closure, mapping direction                                                                          | **PARTIAL** — shapes published and enforced by `bun run skos:check`; no SHACL engine over releases, not in CI |
| P1-014G / R7-024           | w3id redirects for concept/vocab/datatype namespaces                                                                         | **PARTIAL** — config written, `/id/{id}` resolver verified locally; upstream PR unsubmitted                   |
...
| P1-014D | Validate hierarchy and association semantics            | site        | **PARTIAL** — specified in shapes, enforced by `skos:check`                                       |
| P1-014E | Review top concepts                                     | site        | PENDING                                                                                           |
| P1-014F | Validate mapping-property semantics                     | site        | **PARTIAL** — specified in shapes, enforced by `skos:check`                                       |
| P1-014G | Qualify W3ID persistence behavior                       | site        | **PARTIAL** — redirect config + resolver done; upstream PR unsubmitted; no conneg                 |
...
| ID     | Task                                                                             | Owner     | Status                                                                                                    |
| ------ | -------------------------------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------- |
| R7-001 | Publish the SKOS application profile                                             | site      | **PARTIAL** — vocabulary + shapes published; prose profile unwritten                                      |
| R7-002 | Publish SHACL shapes                                                             | site      | **PARTIAL** — published; enforced by an equivalent validator, not a SHACL engine                          |
...
| R7-012 | Implement concept/document content negotiation                                   | site      | PENDING — blocked on per-concept JSON-LD endpoints                                                        |
...
| R7-024 | Test W3ID redirect configuration and persistence separately from representations | site      | **PARTIAL** — resolver behaviours verified locally (301/410/404/405); w3id unverified until the PR merges |
...
| 2026-07-31 | SHACL shapes and the project vocabulary published under `public/profile/`, enforced by `bun run skos:check` (P1-014B/C/D/E/F, R7-002). Concept resolution implemented: `/id/{concept-id}` → 301 current route, 410 for a retired concept, verified against a local Workers runtime; w3id redirect configuration written for upstream submission (P1-014G, R7-024).                                                                                                              |
